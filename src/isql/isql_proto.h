...
void	ISQL_array_dimensions(const QualifiedMetaString&);
...
bool	ISQL_errmsg(IStatus*);
void	ISQL_warning(IStatus*);
void	ISQL_exit_db();
// CVC: Not found.
//int		ISQL_extract(TEXT*, int, FILE*, FILE*, FILE*);
int		ISQL_frontend_command(TEXT*, FILE*, FILE*, FILE*);
bool	ISQL_get_base_column_null_flag(const QualifiedMetaString&, const SSHORT, const MetaString&);
...
void	ISQL_get_domain_default_source(const QualifiedMetaString&, ISC_QUAD*);
SSHORT	ISQL_get_field_length(const QualifiedMetaString&);
...
SLONG	ISQL_get_index_segments(TEXT*, const size_t, const QualifiedMetaString&);
bool	ISQL_get_null_flag(const QualifiedMetaString&, const MetaString&);
...
void	ISQL_print_validation(FILE*, ISC_QUAD*, bool, ITransaction*);
...
processing_state ISQL_print_item_blob(FILE*, const IsqlVar*, ITransaction*, int subtype);
processing_state ISQL_fill_var(IsqlVar*, IMessageMetadata*, unsigned, UCHAR*);
