	...
	if (vector)
	{
		...
		for (ptr = vector->begin(), end = vector->end(); ptr < end; ++ptr)
		{
			if (*ptr)
			{
				...
				fetch_page(true, PageNumber(DB_PAGE_SPACE, *ptr), pag_ids, &window, &page);
				release_page(&window);
			}
		}
	}
 ...
 **************************************/
	...
	while (page_num)
	{
		...
		WIN window(DB_PAGE_SPACE, -1);
		header_page* page = 0;
		fetch_page(true, PageNumber(DB_PAGE_SPACE, page_num), pag_header, &window, &page);
 ...
 **************************************/
	...
	WIN window(pageSpaceId, -1);
	fetch_page(true, PageNumber(pageSpaceId, relPages->rel_index_root), pag_root, &window, &page);
 ...
 **************************************/
	...
	for (ULONG sequence = 0; sequence <= pages; sequence++)
	{
		...
		WIN window(DB_PAGE_SPACE, -1);
		fetch_page(true, PageNumber(DB_PAGE_SPACE, (*vector)[sequence]), pag_transactions, &window, &page);
