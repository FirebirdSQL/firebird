								  ...
								  texttype* tt)
{
	...
	if (statusBuffer[0])
	{
		(Arg::Gds(isc_collation_not_installed) << collationName << charSetName.toString() <<
			Arg::Gds(isc_random) << statusBuffer
		).raise();
	}
	else
		(Arg::Gds(isc_collation_not_installed) << collationName << charSetName.toString()).raise();
}
...
)
{
	...
	{
		gds__log((string("INTL plugin conflict: ") + charSetName.toString() + ":" + collationName +
...
bool IntlManager::validateCharSet(const QualifiedMetaString& charSetName, charset* cs)
{
	...
	unsupportedMsg.printf("Unsupported character set %s.", charSetName.toString().c_str());
