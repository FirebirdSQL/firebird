...
private:
	static void verifyTriggerAccess(thread_db* tdbb, jrd_rel* ownerRelation, TrigVector* triggers,
		Jrd::MetaName userName);
	static void triggersExternalAccess(thread_db* tdbb, ExternalAccessList& list, TrigVector* tvec, const Jrd::MetaName &user);

	void buildExternalAccess(thread_db* tdbb, ExternalAccessList& list, const Jrd::MetaName& user);
...
public:
	...
	Jrd::MetaName triggerName;		// name of request (trigger), if any
