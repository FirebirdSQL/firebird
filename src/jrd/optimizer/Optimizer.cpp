...
void Optimizer::compileLocalTable(StreamType stream)
{
	...
	const auto relation = localTable->getRelation(tdbb, nullptr);
	IndexDescList idxList;
	BTR_all(tdbb, relation, idxList, csb->csb_g_flags & csb_internal);

	MetaId n = idxList.getCount();
	while (n--)
	{
		const auto id = idxList[n].idx_id;
		auto* idv = relation->getPermanent()->lookup_index(tdbb, id, CacheFlag::AUTOCREATE);
		if (idv && idv->getActive() != MET_index_active)
			idv = nullptr;
		if (!idv)
			idxList.remove(n);
