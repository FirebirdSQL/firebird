...
unsigned int  getBIN_BIT_CHAR(UCHAR *value, UCHAR *bit_nr)
{
...
unsigned int  getBIN_BIT_INT(UCHAR *value, unsigned int bit_nr)
{
	unsigned int result, c, i;
	c = *value;
	i = bit_nr;
	...
	impure_value* impure)
{
	...
	unsigned int bit = getBIN_BIT_CHAR(value->dsc_address, value1->dsc_address);
	...
	impure_value* impure)
{
	...
	{
		unsigned int bit = getBIN_BIT_INT(value->dsc_address, i);
		res[j] = bit + '0';
		j++;
	}
	res[bc] = '\0';
