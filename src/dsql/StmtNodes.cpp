...
// ----------------------------------------
// ConstantExprNode implementation
// ----------------------------------------

static RegisterNode<ConstantExprNode> regInitConstantNode({blr_init_constant});

DmlNode* ConstantExprNode::parse(thread_db* tdbb, MemoryPool& pool, CompilerScratch* csb, const UCHAR /*blrOp*/)
{
	ConstantExprNode* node = FB_NEW_POOL(pool) ConstantExprNode(pool);
	node->value = PAR_parse_value(tdbb, csb);
	return node;
}

ConstantExprNode* ConstantExprNode::dsqlPass(DsqlCompilerScratch* /*dsqlScratch*/)
{
	fb_assert(false);
	return this;
}

string ConstantExprNode::internalPrint(NodePrinter& printer) const
{
	StmtNode::internalPrint(printer);
...
void ConstantExprNode::genBlr(DsqlCompilerScratch* dsqlScratch)
{
	dsqlScratch->appendUChar(blr_init_constant);
	GEN_expr(dsqlScratch, value);
}

ConstantExprNode* ConstantExprNode::copy(thread_db* tdbb, NodeCopier& copier) const
{
	ConstantExprNode* node = FB_NEW_POOL(*tdbb->getDefaultPool()) ConstantExprNode(*tdbb->getDefaultPool());
	node->value = value;
	return node;
}

ConstantExprNode* ConstantExprNode::pass1(thread_db* tdbb, CompilerScratch* csb)
{
	ExprNode::doPass1(tdbb, csb, value.getAddress());
	return this;
}

ConstantExprNode* ConstantExprNode::pass2(thread_db* tdbb, CompilerScratch* csb)
{
	impureOffset = csb->allocImpure<impure_value>();
	ExprNode::doPass2(tdbb, csb, value.getAddress());
	return this;
...
const StmtNode* ConstantExprNode::execute(thread_db* tdbb, Request* request, ExeState* /*exeState*/) const
{
	if (request->req_operation == Request::req_evaluate)
	{
		dsc* scalar = EVL_expr(tdbb, request, value);
		impure_value* impure = request->getImpure<impure_value>(impureOffset);
		EVL_make_value(tdbb, scalar, impure);
...
dsc* ConstantExprNode::getImpureDsc(Request* request) const
{
	impure_value* impure = request->getImpure<impure_value>(impureOffset);
