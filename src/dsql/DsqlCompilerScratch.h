...
public:
	...
	void putLocalVariable(dsql_var* variable, const DeclareVariableNode* hostParam,
		const Jrd::MetaName& collationName);
	dsql_var* makeVariable(dsql_fld*, const char*, const dsql_var::Type type, USHORT,
		USHORT, USHORT);
	dsql_var* resolveVariable(const Jrd::MetaName& varName);
	...
	SelectExprNode* findCTE(const Jrd::MetaName& name);
	void clearCTEs();
	...
	DeclareSubFuncNode* getSubFunction(const Jrd::MetaName& name);
	void putSubFunction(DeclareSubFuncNode* subFunc, bool replace = false);

	DeclareSubProcNode* getSubProcedure(const Jrd::MetaName& name);
...
public:
	...
	Firebird::Stack<Jrd::MetaName*> labels;	// Loop labels
	...
	Jrd::MetaName package;			// package being defined
