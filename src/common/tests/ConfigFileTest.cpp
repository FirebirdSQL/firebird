...
include /a/b/c/d/f.d
	...
	BOOST_CHECK_EXCEPTION(ConfigFile file({}, text.data(), 0), Firebird::status_exception,
		[](const Firebird::status_exception& ex)
		{
			// Cannot check text due to fb_master_interface been not implemented, so fb_interpret cannot be used
			return true;
			// return TestsUtils::checkErrorMessage(ex, "Invalid include operator in Passed text for </a/b/c/d/f.d> File to include not found");
		});
}
